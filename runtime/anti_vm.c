 ...
 * (see the file header). */
static int karity_avm_check_hypervisor_bit(void)
{
    uint32_t regs[4];
    karity_avm_cpuid(1, 0, regs);
    return (regs[2] & 0x80000000u) != 0; /* ECX */
}
...
static int karity_avm_check_hypervisor_vendor(void)
{
    uint32_t regs[4];
    karity_avm_cpuid(0x40000000u, 0, regs);
    return regs[1] != 0; /* EBX: first 4 bytes of the vendor string */
}
...
static int karity_avm_check_vmware_backdoor(void)
{
    ...
    is_vmware = !g_avm_vmware_faulted && ebx == 0x564D5868u;
...
static int karity_avm_check_cpuid_timing(void)
{
    uint32_t regs[4];
    uint64_t t0, t1;
...
uint64_t karity_anti_vm_scan(void)
{
    ...
    if (karity_avm_check_hypervisor_bit())    taint |= KARITY_AVM_TAINT_HV_BIT;
    if (karity_avm_check_hypervisor_vendor()) taint |= KARITY_AVM_TAINT_HV_VENDOR;
    if (karity_avm_check_vmware_backdoor())   taint |= KARITY_AVM_TAINT_VMWARE_PORT;
    if (karity_avm_check_cpuid_timing())      taint |= KARITY_AVM_TAINT_CPUID_TIMING;
